...
// 把要使用的变量申明在这里，这样describe里的各个函数都可以使用了
...
describe('Collapse.vue', () => {
  beforeAll(() => {
    ...
    headers = wrapper.findAll('.wow-collapse-item__header')
    contents = wrapper.findAll('.wow-collapse-item__wrapper')
  ...
  test('disabled 的内容应该没有反应', async () => {
    // 清理掉onChange上的mock数据，因为前面有调用onChange两次，里面已经有数据（调用次数，调用参数）了，我们需要把其重置
    onChange.mockClear()
